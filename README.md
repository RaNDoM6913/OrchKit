...
- Snapshot-bound review import from the exact frozen export report path; report files are no-follow/regular-file checked, capped at 256 KiB, findings/uncertainty are structurally bounded, and raw report SHA-256/bytes are recorded. Failed verification/review becomes `NEEDS_FIX` and is picked up by a **new** ChatGPT run.
